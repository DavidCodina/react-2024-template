...
export const Modal = ({
  ...
  const header = (
    <div className='flex items-center justify-between border-b border-neutral-200 p-3'>
      <h3 className='m-0 text-xl font-bold leading-none text-blue-600'>
        {title}
      </h3>
    </div>
  )
  ...
  const X = (
    <button
      onClick={onClose}
      className='absolute right-1 top-1 inline-flex items-center rounded-lg bg-transparent text-neutral-400 hover:text-blue-500'
    >
      <svg
        height='2em'
        width='2em'
  ...
  return (
    <div
      className='fixed inset-0 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-black bg-opacity-50'
      style={{ zIndex: 9999 }}
    >
      <div
